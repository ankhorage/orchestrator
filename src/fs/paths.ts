...
export function resolveProjectPath(projectRoot: string, relativePath: string): string {
  if (!relativePath || path.isAbsolute(relativePath)) {
    throw new Error(`Expected a relative project path, received "${relativePath}".`);
  }
  ...
  if (!normalizedResolved.startsWith(normalizedRoot) && resolved !== path.resolve(projectRoot)) {
